...
@app.task
def create_record(currency_dollar, currency_euro, currency_yen, currency_yuan):
    ...
    data.yen_rate = currency_yen
    data.yuan_rate = currency_yuan
...
@app.task
def check_rate(currency_dollar, currency_euro, currency_yen, currency_yuan):
    ...
    max_value = list(ExchangeData.objects.filter(time_rate__range=(
        ...
        f'{datetime.now().replace(second=0, microsecond=0)}')).order_by('-dollar_rate').values('id', 'dollar_rate', 'euro_rate', 'time_rate'))[0]
        ...
        if check_status[user_true]['sending_status'] is True:
            if ((currency_dollar / max_value['dollar_rate']) * 100 - 100 <= -check_status[user_true][
                ...
                if max_value['id'] <= min_value['id']:
                    bot.send_message(check_status[user_true]['username'],
                                     ...
                                     f' рубль вырос на '
                                     f'{abs(round(((currency_dollar / max_value["dollar_rate"]) * 100 - 100), 3))}%, '
                ...
                bot.send_message(check_status[user_true]['username'],
                                 f'Курс рубля к доллару вырос больше чем на {int(check_status[user_true]["percent_user"])}% '
                                 f'по сравнению с курсом на {max_value["time_rate"].replace(tzinfo=None)}, '
                                 f'рубль вырос на {abs(round(((currency_dollar / max_value["dollar_rate"]) * 100 - 100), 3))}%, '
                                 f'значение курса изменилось с {max_value["dollar_rate"]} до {currency_dollar}')
